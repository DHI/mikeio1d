...
* **EPANET (`.res`)**: a pipe's flow is read at the pipe's start, `(pipe_id, 0.0)`. With the `.inp` beside the `.res` (same folder and stem), or passed in `companions`, it can also be read at the pipe's end; the two give the same series. A `.resx` beside the `.res` adds tank volumes and pump energy. Pass `companions=[]` to read the `.res` alone.
